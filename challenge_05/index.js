const USER_LIST = `PYmt8hd,jdcrLI,jdcrli@gmail.com,27,London
...
const verificaciones = [];
  ...
  // comprobar id: existe y es alfanumérica
const idArr = id.toLowerCase().split("")
id && idArr.every( (el) => alfaNumerico.includes(el)) ? verificaciones.push(true) : verificaciones.push(false);

//comprobar username: existe y es alfanumérico
const usernameArr = username.toLowerCase().split("")
username && usernameArr.every((el) => alfaNumerico.includes(el)) ? verificaciones.push(true) : verificaciones.push(false)

// comprobar email: existe y es válido (sigue el patrón user@dominio.com)
const emailArr = email.split("")
emailArr && emailArr.includes("@") && email.includes(".com") ? verificaciones.push(true) : verificaciones.push(false)


// age age: es opcional pero si aparece es un número
const ageArr = age.split("")

if(age){
    ageArr.every( (el) => numerico.includes(el)) ? verificaciones.push(true) : verificaciones.push(false)
}else{
    verificaciones.push(true)
}

//comprobar location: es opcional pero si aparece es una cadena de texto
const locationArr = location.toLowerCase().split("") 
if(location){
    locationArr.every( (el) => alfa.includes(el)) ? verificaciones.push(true) : verificaciones.push(false)
}else{
    verificaciones.push(true)
}

!verificaciones.every( (el) => el === true) && invalidUsers.push(username[0])


});
console.log(invalidUsers.join(""));